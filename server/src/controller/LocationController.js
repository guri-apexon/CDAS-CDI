...
exports.searchLocationList = function (req, res) {
  try {
    ...
    const searchQuery = `SELECT loc_id,loc_typ,ip_servr,port,usr_nm,pswd,cnn_url,data_strc,active,extrnl_sys_nm,loc_alias_nm from cdascdi1d.cdascdi.location 
            WHERE LOWER(loc_typ) LIKE '%${searchParam}%' OR 
            LOWER(loc_alias_nm) LIKE '%${searchParam}%'
            `;
    Logger.info({
  ...
  try {
    ...
    if(type) {
        switch(type) {
            ...
            break;
            default:
                searchQuery = `SELECT ${select} from cdascdi1d.cdascdi.location where loc_typ = '${type}'`;
        }
    }
    Logger.info({
      message: "locationList",
