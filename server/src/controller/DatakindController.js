...
exports.createDataKind = async (req, res) => {
  try {
    ...
    const insertQuery = `INSERT INTO ${config.DB_SCHEMA_NAME}.datakind
...
exports.getDatakindList = function (req, res) {
  try {
    let searchQuery = `SELECT datakindid,datakindid as value,CONCAT(name, ' - ', extrnl_sys_nm) as label, name from ${config.DB_SCHEMA_NAME}.datakind where active= $1 order by label asc`;
