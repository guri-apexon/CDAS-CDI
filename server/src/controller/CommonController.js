...
module.exports = {
  ...
  addPackageHistory: function (package, user_id, column, old_val, new_val) {
    return new Promise((resolve, reject) => {
      ...
      DB.executeQuery(
        `SELECT version from ${config.DB_SCHEMA_NAME}.datapackage_history
      WHERE datapackageid = '${package.datapackageid}' order by version DESC limit 1`
        ...
        DB.executeQuery(addHistoryQuery, values).then(async (response) => {
          ...
          const addAuditLogQuery = `INSERT INTO ${config.DB_SCHEMA_NAME}.dataflow_audit_log(dataflow_audit_log_id, dataflowid, datapackageid, audit_vers, audit_updt_dt, usr_id, attribute, old_val, new_val) VALUES($1, $2, $3, $4, $5, $6, $7, $8, $9)`;
          const auditValues = [
            auditId,
            package.dataflowid,
            package.datapackageid,
