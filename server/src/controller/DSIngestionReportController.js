...
exports.getDatasetIngestionReportMetrics = (req, res) => {
  try {
    ...
    const searchQuery = `select * from fn_get_dataset_metric('${id}')`;
...
exports.getDatasetIssueTypes = (req, res) => {
  try {
    ...
    const searchQuery = `select * from fn_get_dataset_issue_summary('${id}')`;

    Logger.info({ message: "fn_get_dataset_issue_summary" });
...
exports.getFileTransferHistory = (req, res) => {
  try {
    ...
    --(lead(processtrnx, 1, 0) OVER (PARTITION BY datasetid ORDER BY lastsucceeded DESC,externalid DESC) - modified_records) AS unchanged_records
	  coalesce(processtrnx,0) -(coalesce(new_records,0)+ coalesce(modified_records,0)) AS unchanged_records
